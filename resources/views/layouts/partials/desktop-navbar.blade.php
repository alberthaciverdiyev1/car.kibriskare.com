...
<header class="hidden md:block bg-white border-b border-gray-200 sticky top-0 z-30">
  <div class="px-5 mx-auto h-17.5 flex items-center justify-between gap-4">
    ...
    <nav class="hidden md:flex items-center space-x-5 lg:space-x-7 text-[15px] font-medium">
      <a href="{{ route('listing', ['deal_type' => 'sale']) }}" class="{{ request('deal_type') === 'sale' || request()->is('*satilik*') ? 'text-[var(--primary)] font-semibold' : 'text-gray-700 hover:text-[var(--primary)]' }} transition">
        {{ __('navbar.sale') }}
      </a>
      <a href="{{ route('listing', ['deal_type' => 'rent_daily']) }}" class="{{ request('deal_type') === 'rent_daily' || request()->is('*kira*') || request()->is('*rent*') ? 'text-[var(--primary)] font-semibold' : 'text-gray-700 hover:text-[var(--primary)]' }} transition">
        {{ __('navbar.rent') }}
      </a>
      <a href="{{ route('autosalons.index') }}" class="{{ request()->is('*avtosalon*') || request()->is('*autosalon*') || request()->is('*agenc*') ? 'text-[var(--primary)] font-semibold' : 'text-gray-700 hover:text-[var(--primary)]' }} transition">
        {{ __('navbar.autosalons') }}
      </a>
      <a href="{{ route('contact') }}" class="{{ request()->is('*iletisim*') || request()->is('*contact*') ? 'text-[var(--primary)] font-semibold' : 'text-gray-700 hover:text-[var(--primary)]' }} transition">
        {{ __('navbar.contact') }}
      </a>
    </nav>
    ...
    <div class="flex items-center space-x-2.5 sm:space-x-3.5">
      ...
      <!-- Favorites -->
      <a href="{{ route('favorites') }}" class="relative text-gray-700 hover:text-[var(--primary)] p-2 rounded-xl transition inline-flex items-center justify-center hover:bg-gray-50" title="{{ __('navbar.favorites') }}">
        <i class="fa-regular fa-heart text-xl text-rose-500"></i>
        <span id="favorites-count" class="absolute -top-1 -right-1 bg-[var(--primary)] text-white text-[10px] min-w-[16px] h-4 px-1 flex items-center justify-center rounded-full font-semibold shadow-sm">0</span>
      </a>

      <!-- Compares -->
      <a href="{{ route('compares') }}" class="relative text-gray-700 hover:text-[var(--primary)] p-2 rounded-xl transition inline-flex items-center justify-center hover:bg-gray-50" title="{{ __('navbar.compare') }}">
        <i class="bi bi-arrow-left-right text-xl text-gray-700"></i>
        <span id="compares-count" class="absolute top-0 -right-1 bg-[var(--primary)] text-white text-[10px] min-w-[16px] h-4 px-1 flex items-center justify-center rounded-full font-semibold shadow-sm">0</span>
      </a>
        ...
        <div id="navCurrencyDropdown"
          ...
          @foreach($currencySymbols as $cCode => $cSym)
            <a href="{{ route('currency.switch', ['code' => $cCode]) }}"
               class="flex items-center justify-between px-3.5 py-2 text-xs font-semibold {{ $currentCurrency === $cCode ? 'text-[var(--primary)] bg-emerald-50/60 font-semibold' : 'text-gray-700 hover:bg-gray-50' }} transition">
              <span class="flex items-center gap-2">
                <span class="w-4 text-center font-semibold text-gray-400">{{ $cSym }}</span>
                <span>{{ $cCode }}</span>
              </span>
              @if($currentCurrency === $cCode)
                <i class="bi bi-check2 text-sm text-[var(--primary)]"></i>
              @endif
            </a>
          @endforeach
        </div>
        ...
        <div id="navLangDropdown"
          ...
          @foreach($languages as $lKey => $lData)
            <a href="{{ route('lang.switch', ['lang' => $lKey]) }}"
               class="flex items-center justify-between px-3.5 py-2 text-xs font-semibold {{ $currentLocale === $lKey ? 'text-[var(--primary)] bg-emerald-50/60 font-semibold' : 'text-gray-700 hover:bg-gray-50' }} transition">
              <span class="flex items-center gap-2">
      ...
      <!-- Add Car Button -->
      <a href="{{ route('add-car') }}" class="hidden sm:flex items-center px-4 py-2 bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white rounded-xl font-semibold text-xs sm:text-sm transition shadow-sm">
      ...
      <div class="relative">
        @auth
          ...
          <div id="navUserDropdown" class="hidden absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-50">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-emerald-50 hover:text-[var(--primary)]">
              <i class="bi bi-grid mr-3 text-gray-400"></i> {{ __('navbar.dashboard') }}
            </a>
            <a href="{{ route('profile') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-emerald-50 hover:text-[var(--primary)]">
              <i class="bi bi-person mr-3 text-gray-400"></i> {{ __('navbar.my_profile') }}
            </a>
            <a href="{{ route('my-properties') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-emerald-50 hover:text-[var(--primary)]">
              <i class="bi bi-folder-check mr-3 text-gray-400"></i> {{ __('navbar.my_properties') }}
            </a>
            <a href="{{ route('favorites') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-emerald-50 hover:text-[var(--primary)]">
              <i class="bi bi-heart mr-3 text-gray-400"></i> {{ __('navbar.my_favorites') }}
            </a>

            @if(auth()->user()->is_admin ?? false)
              <div class="border-t border-gray-100 my-1"></div>
              <a href="{{ route('filament.admin.pages.dashboard') }}" class="flex items-center px-4 py-2 text-sm text-[var(--primary)] hover:bg-emerald-50 font-medium">
        ...
        @else
          <a href="{{ route('login') }}" class="flex items-center px-3.5 py-2 border border-gray-200 rounded-xl text-gray-700 hover:border-[var(--primary)] hover:text-[var(--primary)] text-sm font-semibold transition bg-white shadow-2xs">
